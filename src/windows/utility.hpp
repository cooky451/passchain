...
{
	std::wstring buffer(GetWindowTextLengthW(hwnd), wchar_t());
	GetWindowTextW(hwnd, &buffer[0], static_cast<int>(buffer.size() + 1));
	return toUtf8(buffer);
}
...
void centerWindowOnScreen(HWND hdlg)
{
	RECT rc;
	GetWindowRect(hdlg, &rc);

	auto x = (GetSystemMetrics(SM_CXSCREEN) - (rc.right - rc.left)) / 2;
	auto y = (GetSystemMetrics(SM_CYSCREEN) - (rc.bottom - rc.top)) / 2;

	SetWindowPos(hdlg, nullptr, x, y, 0, 0, SWP_NOSIZE);
}
